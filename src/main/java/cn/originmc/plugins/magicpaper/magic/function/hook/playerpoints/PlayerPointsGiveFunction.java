...
public class PlayerPointsGiveFunction extends NormalFunction {
    @Override
    public FunctionResult whenFunctionCalled(SpellContext spellContext, List<FunctionResult> args) {
        if (args.size() < 2) {
            return new ErrorResult("FUNCTION_ARGS_ERROR", "PlayerPointsGive don't have enough args.");
        }
        ...
        if (!VariableUtils.tryInt(money)) {
            return new ErrorResult("TYPE_ERROR", "The second arg must be number.");
        }
        return new BooleanResult(PlayerPointsManager.givePoints(player.getUniqueId(), Integer.parseInt(money)));
    }

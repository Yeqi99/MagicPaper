...
public class GuiNextPageFunction extends NormalFunction {
    @Override
    public FunctionResult whenFunctionCalled(SpellContext spellContext, List<FunctionResult> args) {
        if (args.size()<2){
            return new ErrorResult("ARGS_ERROR", "GuiNextPageFunction need 2 args");
        }
        FunctionResult arg0=args.get(0);
        FunctionResult arg1=args.get(1);
        if (!(arg0 instanceof PlayerResult)){
            return new ErrorResult("ARGS_ERROR", "The first arg of GuiNextPageFunction must be a player");
        }
        if (!(arg1 instanceof StringResult)){
            return new ErrorResult("ARGS_ERROR", "The second arg of GuiNextPageFunction must be a string");
        }
        ...
        MagicPaper.getMagicGuiManager().getGui(player, guiId).nextPage(player);
